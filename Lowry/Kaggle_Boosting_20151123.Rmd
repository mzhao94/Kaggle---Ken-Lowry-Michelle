...
####Boosting Model. Uses data created above. 
...
boost.training = gbm(ALSFRS_slope ~ ., data = merged.data.subset, distribution = "gaussian", n.trees = 1000, interaction.depth = 2)

#Validation RMSE
slopes.boost = predict(boost.training, newdata = validation.subset.with.target[,-2], n.trees = 1000)
valid.rmse = mean((slopes.boost-validation.subset.with.target$ALSFRS_slope)^2)^.5
valid.rmse 
